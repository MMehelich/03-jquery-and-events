...
articleView.handleAuthorFilter = function() {
  $('#author-filter').on('change', function() {
    ...
    if ($(this).val()) {
      ...
      let $whereToGo = $(this).val();
      $('article').hide();
      $('article[data-author="' + $whereToGo + '"]').fadeIn(750);
    } else {
...
articleView.handleCategoryFilter = function() {
  ...
  $('#category-filter').on('change', function() {
    if ($(this).val()) {   
      let $whereToGo = $(this).val();
      $('article').hide();
      $('article[data-category="' + $whereToGo + '"]').fadeIn(750);
    } else {
      $('article').show();
      $('.template').hide();
    }
...
// STRETCH GOAL
articleView.resetTeasers = function() {
  $('#articles').on('click', '.show-less', function(event) {
    event.preventDefault();
    $(this).siblings('.article-body').find('*:nth-of-type(n+2)').hide();
    $(this).hide();
